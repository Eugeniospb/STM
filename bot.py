...
def is_allowed_user(user_id: int, username: str = None) -> bool:
    return user_id in ALLOWED_IDS or (username and username.lower() in [u.lower() for u in ALLOWED_USERNAMES])
...
async def process_document(bot, document) -> tuple:
    ...
    # DOCX -> извлекаем текст
    elif "wordprocessingml" in mime_type or (document.file_name and document.file_name.endswith('.docx')):
        try:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_allowed_user(user.id, user.username):
    ...
    async def error_handler(update, context):
        ...
        try:
            if update and update.effective_message:
                await update.effective_message.reply_text("⚠️ Ошибка обработки. Попробуйте ещё раз.")
        except: pass
